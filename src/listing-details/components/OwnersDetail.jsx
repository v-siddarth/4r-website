...
function OwnersDetail({ wasteDetails }) {
  ...
  const OnMessageOwnerButtonClick = async () => {
    ...
    try{
      ...
      await CreateSendBirdUser(ownerUserId, wasteDetails?.userName, wasteDetails?.userImageUrl);
      ...
      const channelResponse = await CreateSendBirdChannel([userId, ownerUserId], wasteDetails?.listingTitle);
  ...
  return (
    <div className='p-10 border rounded-xl shadow-md mt-7'>
      ...
      <img src={wasteDetails?.userImageUrl} className='w-[70px] h-[70px] rounded-full'/>
      <h2 className='mt-2 font-bold text-xl'>{wasteDetails?.userName}</h2>
      <h2 className='mt-2 text-gray-500'>{wasteDetails?.createdBy}</h2>
